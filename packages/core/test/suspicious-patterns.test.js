 ...
 *   ^\s+#{1,6}\s+(?:System|Assistant|User|Human|AI|Bot|Admin|Developer|Operator|Moderator)\s*:
 *
 * - H1-H6 (all standard markdown heading levels).
 * - One of 10 role keywords (case-insensitive via `i` flag).
 ...
 * extension (8 -> 10 roles) so the TRANSCRIPT_NOISE 3-hit banner-suppression
 * rule keeps applying.
 */
...
describe("Markdown heading impersonation pattern (v1.10.0 Theme B, v1.11.0 Theme D H4, v1.12.0 Theme E H5+H6, v1.14.0 mid-03 Operator+Moderator)", () => {
  describe("positive matches: 60 role x level combinations all fire warning (H1-H6 x 10 roles)", () => {
