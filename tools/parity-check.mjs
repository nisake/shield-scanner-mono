...
async function main() {
  ...
  const totalFixtures = fixtures.length + archive.count + pdfStruct.count;
  const totalMcp = totalMcpFindings + archive.totalMcp + pdfStruct.totalMcp;
  const totalWeb = totalWebFindings + archive.totalWeb + pdfStruct.totalWeb;

  console.log(
    `${totalFixtures} fixtures scanned (${fixtures.length} text + ${archive.count} archive + ${pdfStruct.count} pdf-struct) / MCP findings: ${totalMcp} / Web findings: ${totalWeb} / parity drift: ${drift}`,
  );
